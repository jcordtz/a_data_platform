...
>[!Note]
> There is supporting slidedeck to be found and the images used in this repo can also be found in that slidedeck.
...
||[Loading & Perfomance](./DataOps/Dataloads.md)|[Supporting services](./Supporting_Data_Services/SupportingDataServices.md)|[Naming Standards](./DataModelling/NamingStandard.md)|
...
||[Risk-assessment](./Security/Exit-and-risc-strategies.md)|[Meta data handling](./DataOps/Meta-management.md)|[Soverignity](./Security/Soverignity.md)|||

## Background

The discussion in this document is about what data can be used for and how to ensure that you are always prepared for any new situation where data is needed. And that this approach is also done in a **compliant** as well as a **governed** way reflecting your guidelines and politics of data usage.
...
The data platform discussed is based on PaaS and/or SaaS services. In the chapter “Implementations”, different ways of doing this is discussed with the usage of different PaaS or SaaS services. This is indicated by the trafic lights - go right, not left :smile:

This also translates to the way you ask for *things* in a cloud - ask how you can accomplish the desired functionality and thereby get the most appropiate service. Do not find a product and then try to make that handle your functional requirements. In other words - it is not a requirement to use a relational database like *MS SQL Server* to have a *data warehouse*, but it could be a very appropiate place to store your data because you want to have a suitable engine to handle your SQL queries.
...
Based on the above we must ensure that the data we provide to the solutions created fits exactly what the user has access to, so that the grounding and hence the vector database contains only this. This can be done through the *publish area* as this area represents datasets with the exact content. So the task is to "load" the vector databases being used - and make sure these are stopped/removed after usage.
...
So what we are discussing is if and how we could take an approach where the relevant "tables" in their system-landscape is fronted by an agent each. Each of these agents describes the capabilities of the individual table like what does it contain, how can be used, how can it be combined with other pieces of information and so on.
...
A way of establishing a secure development environment is to use what is called a Project room in this document. These project rooms represent an isolated environment normally owned by a unique AD-Group (one or more).
...
In the project room you will find a "mini" data platform. This means that you will find bronze, silver and gold areas, most likely as directories (not storage accounts) and all the tools in the data platform and a desktop to uses these tools.

You might decide to have a few different types of project rooms - i.e. one for the ingest process, on for the transform process and one for the publishing part. But it should be at such a level, do not have a project room type per tool !!!
...
This development when "checked in" is then captured by pipelines that then handles the code depolyment to the overall data platform using a CI/CD process. ③ on the figure.
...
From here, the data elements that will be used in a common data platform move to that area, and the remaining data elements are passed to the respective transform's areas - can also be referred to as **receive areas**. There can easily be several of such areas per department.
